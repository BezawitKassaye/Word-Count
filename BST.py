class TreeNode:
    ...
    def __str__(self):
        return f"""{self.key}: {self.value}; parent = {str(self.parent.key) if self.parent else str(None)};
left = {str(self.left_child.key) if self.left_child else str(None)};
right = {str(self.right_child.key) if self.right_child else str(None)}"""
